...
function getCookie(token) {
	return new Promise(async (resolve, reject) => {
		...
		try {
			...
			// 短信阻塞
			const promiseA = new Promise((resolveQ, rejectQ) => {
				resolveHandle = resolveQ;
				rejectHandle = rejectQ;
			});
			// 风控校验阻塞
			const promiseB = new Promise((resolveQ, rejectQ) => {
				resolveHandleB = resolveQ;
				rejectHandleB = rejectQ;
			});
			...
			page.on('request', async interceptedRequest => {
				if (interceptedRequest.url() === LOGIN.url) {
					...
					if (
						options.headers &&
						(options.headers.cookie == null || options.headers.Cookie == null)
					) {
				...
				if (interceptedRequest.url().indexOf('checkLoginAccount') > -1) {
					...
					if (
						resJson.code === 'SLR_ERR_0005' ||
						resJson.code === 'SLR_ERR_0006'
					) {
						// 登录成功
						let loginUrl = '';
						if (resJson.code === 'SLR_ERR_0005') {
							loginUrl = await page.evaluate(token => {
								return (
									srs_prefix_domain +
									'smsLogin/checkLoginToken.do?type=1&rememberMe=true&acessToken=' +
									token +
									'&detect=' +
									getDetect() +
									'&dfpToken=' +
							...
							loginUrl = await page.evaluate(token => {
								return (
									srs_prefix_domain +
									'smsLogin/checkTokenRegAndLogin.do?type=1&rememberMe=true&acessToken=' +
									token +
									'&detect=' +
									getDetect() +
									'&dfpToken=' +
						...
						const extraCookie = loginRes.headers['set-cookie']
							.map(cookie => {
					...
					if (response.url().indexOf('needVerifyCode') > -1) {
						...
						if (data.msg.indexOf('风控返回有风险') > -1) {
							...
							const slideBtn = await page.evaluate(() => {
								const info = document
									.querySelector('.dt_child_content_knob')
							...
							await page.waitFor(
								() =>
									document.querySelector('#dt_notice').innerText ===
									'请按住滑块拖到最右边'
							);
							...
							await page.mouse.move(axleX + distance * ratio, axleY, {
								steps: 10
							});
			...
			const msgCode = msgCodeStr.replace(/[^0-9]/gi, '');
